...
class Comment extends ContextSource {
	...
	public function __construct( CommentsPage $page, $context = null, $data ) {
		...
		$this->commentVote; // @TODO why 4 vars?
	...
	function getVoteLink( $voteType ) {
		...
		if ( $this->getUser()->isLoggedIn() ) {
			...
			// where to begin...
			$returnToPageName = $this->page->title;
			if ( $returnToPageName instanceof Title ) { // @TODO should always be, error handling should be in CommentsPage
	 ...
	 *
	 * @param $block_list: @TODO document
	 *
	 * @return string: html
	 */
	function display( $block_list ) {
		...
		if ( $this->userID != 0 ) {
			...
			if ( !array_key_exists( $this->username, $AFBucket ) ) {
				$AFBucket[$this->username] = $AFCounter; // @TODO
				$AFCounter++;
			}

			$anonMsg = $this->msg( 'comments-anon-name' )->inContentLanguage()->plain();
			$CommentPoster = $anonMsg . ' #' . $AFBucket[$this->username];
		...
		if (
			...
			!( in_array( $this->userID, $block_list ) )
		) {
		...
		if ( in_array( $this->username, $block_list ) ) {
