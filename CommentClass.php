...
class Comment extends ContextSource {
	...
	static function add( $text, CommentsPage $page, User $user, $parentID ) {
		...
		// Add a log entry.
		$this->log( 'add', $user, $page->id, $commentId, $text );
	...
	function delete() {
		...
		$this->log( 'delete', $this->getUser(), $this->page->id, $this->id );
	 ...
	 */
	private function log( $action, $user, $pageId, $commentId, $commentText = null ) {
		global $wgCommentsInRecentChanges;
