...
class Comment extends ContextSource {
	...
	public function __construct( CommentsPage $page, $context = null, $data ) {
		...
		//$this->currentScore = $data['Comment_Score'];
		//$this->plusCount = $data['CommentVotePlus'];
		//$this->minusCount = $data['CommentVoteMinus'];
		...
		//$key = wfMemcKey( 'comment', 'score', $this->id );
		//$score = $wgMemc->get( $key );
		//if ( $score === false ) { // only if there is no caching
		//	$score = $this->getScore();
		//}
	...
	static function add( $text, CommentsPage $page, User $user, $parentID ) {
		...
		$userPoints = ( isset( $row->stats_total_points ) ? number_format( $row->stats_total_points ) : 0 );
	...
	function vote( $value ) {
		...
		// update cache voted list NEVER USED?!
	...
	/*
	function getScore() {
		$plusCount = $this->getVoteCount( 1 );
		$minusCount = $this->getVoteCount( -1 );
