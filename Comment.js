...
var Comment = {
		...
		if ( confirm( message ) ) {
			jQuery.ajax( {
				...
				if ( response.commentblock.ok ) {
					jQuery( '#comment-' + commentID ).hide( 300 );
					jQuery( 'a.comments-block-user[data-comments-user-id=' + userID + ']' ).parents( '.c-item' ).hide( 300 );
					jQuery( '#ignore-' + commentID ).show( 300 );
					jQuery( 'a.comments-block-user[data-comments-user-id=' + userID + ']' ).parents( '.c-item' ).prev().show( 300 );
				}
			} );
		}
	},
	...
	vote: function( commentID, voteValue ) {
		jQuery.ajax( {
			...
			jQuery( '#comment-' + commentID + ' .c-score' ).html( response.commentvote.html );
			jQuery( '#comment-' + commentID + ' .c-score' ).html( jQuery( '#comment-' + commentID + ' .c-score' ).text() ); // unescape
		} );
	},
