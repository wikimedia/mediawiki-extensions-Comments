...
class CommentsHooks {
	...
	public static function onLoadExtensionSchemaUpdates( $updater ) {
		...
		$lowerCaseColumns = in_array( $dbType, [ 'postgres', 'sqlite' ] );
		// Actor support
		if ( !$db->fieldExists( 'Comments', $lowerCaseColumns ? 'comment_actor' : 'Comment_actor', __METHOD__ ) ) {
			// 1) add new actor columns
			$updater->addExtensionField( 'Comments', $lowerCaseColumns ? 'comment_actor' : 'Comment_actor', "$dir/patches/actor/add-Comment_actor{$patchFileSuffix}.sql" );
		...
		if ( !$db->fieldExists( 'Comments_Vote', $lowerCaseColumns ? 'comment_vote_actor' : 'Comment_Vote_actor', __METHOD__ ) ) {
			// 1) add new actor columns
			$updater->addExtensionField( 'Comments_Vote', $lowerCaseColumns ? 'comment_vote_actor' : 'Comment_Vote_actor', "$dir/patches/actor/add-Comment_Vote_actor{$patchFileSuffix}.sql" );
			// 2) add the corresponding indexes
