...
class CommentsPage extends ContextSource {
    ...
    public function getCommentList( $page ) {
        ...
        if ( $this->orderBy == 0 ) {
            usort( $comments, array( 'Comment', 'sortCommentList' ) ); // @TODO
        }
     ...
     */
    function display() {
        global $wgExtensionAssetsPath, $wgMemc, $wgUserLevels;
...
*/
        ...
        if ( $comments ) {
            ...
            foreach ( $comments as $comment ) {
                $output .= $comment->display( $block_list );
            }
            $output .= $pager;
        }
    ...
    function displayForm() {
        ...
        if ( !$this->getUser()->isAllowed( 'comment' ) ) {
            ...
            $output .= '<input type="hidden" name="pid" value="' . $this->id . '" />' . "\n";
